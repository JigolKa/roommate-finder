...
class googleimagesdownload:
    ...
    def _image_objects_from_pack(self, data, raw_google_data_filepath=None):
        if raw_google_data_filepath is not None:
            with open(raw_google_data_filepath, "w") as f:
    ...
    def download_extended_page(self, url, chromedriver, raw_google_data_filepath=None):
        ...
        images = self._image_objects_from_pack(
            self._extract_data_pack_extended(source), raw_google_data_filepath)
        ...
        for chunk in ajax_data:
            ...
            images += self._image_objects_from_pack(
                self._extract_data_pack_ajax(chunk), sub_raw_google_data_filepath)
            extract_id += 1
        ...
        for pky in prefix_keywords:  # 1.for every prefix keywords
            for sky in suffix_keywords:  # 2.for every suffix keywords
                ...
                while i < len(search_keyword):  # 3.for every main keyword
                    ...
                    if raw_google_data:
                        google_data_filename = f'{pky}{search_keyword[i]}{sky}.json'.replace(
                            '/', '_')
                        raw_google_data_filepath = os.path.join(
                            'raw_google_data', google_data_filename)
                    else:
                        raw_google_data_filepath = None

                    if limit < 101:
                        images, tabs = self.download_page(
                            url, raw_google_data_filepath)  # download page
                    else:
                        images, tabs = self.download_extended_page(
                            url, arguments['chromedriver'], raw_google_data_filepath)
